...
- **french-52 only.** A deck must say `deckType: "french-52"`; its well-known ids map straight to engine codes (`hearts-K` → `Kh`, `spades-10` → `Ts`). `value` is ignored: Calliope ranks by rank id. The first two jokers become `*1` and `*2`, dealt only when jokers are wild; a deck without jokers is still complete. Anything else is refused with a plain sentence.
...
- **Starter decks** are the Classic Deck (the default) and Classic Deck 54, the same style with its two jokers. They ship unzipped under `packages/web/public/decks/<folder>/` and are served as plain files, fetched once and cached. The build checks each (complete french-52, a licence, every picture present: PNGs at the stated size, SVGs with an `<svg>` root) and fails otherwise. The first folder in `starterDecks({ order })` in `vite.config.ts` is the default.
...
- While someone else is betting and the player is still in, the buttons are live but dashed, and set what they will do when the action reaches them: **Check / fold** (Fold when facing a bet), **Check** or **Call n**, **Min bet** / **Min raise** (`preActions.ts`). A set choice is filled `--ink` with a tick. A call or raise is dropped if the price changes, with a note; choices last one street. "Waiting for Bob" moves into the bar's top rule. When the turn comes the choice is sent at once, and the bar reads "Done, as you chose" instead of offering the buttons twice.
...
- Keyboard: `F` fold, `C` check/call, `R` open bet panel by default, or `A S D`, `J K L` or any three letters or digits, chosen per device on the profile page (`keys.ts`). Before the player's turn the same keys set the choices above. Digits type into the amount, `Enter` confirm, `Esc` cancel. Shortcuts are printed in micro small caps under each button on desktop.
- Controller (`gamepad.ts`, any pad the browser's Gamepad API knows, standard mapping): the D-pad or left stick moves focus to the nearest control in that direction (`padNav.ts`: controls in line first), on every screen. A presses; on a number, slider or list, A picks it up to change with the D-pad (ringed `--red`) and A again puts it down, as Enter. B goes back. X and Y fold and bet/raise by sending the player's own keys, so they work ahead of the turn too. LB/RB step a number by ten, Start opens the table menu, the right stick scrolls. While a pad is in use the page carries `data-input="pad"` and every focused control gets a 3px `--focus` ring; a pointer hands it back. When the turn comes, focus goes to Check/Call. `e2e/controller.mjs` plays a whole night on a fake pad and checks every control on the lobby, table, report and profile is reachable.
